...
module.exports = {
  ...
  /**
   * Write CSV records to destination path on file system 
   */
  writeFiles: function(csvArr, path, done) {
    if (!fs.existsSync(path)) {
      return done('Destination directory does not exist');
    }
    let count = 0;
    for (const data of csvArr) {
      fs.writeFileSync(`${path}/consolidated-${count++}.csv`, data);
    }
