  ...
  describe('.makeNeighbours', () => {
    ...
    it('sets a property\'s ascending neighbour', done => {
      db.Report.find({ 'Location Address': { $in: ['405 FAKE ST NW', '371 FAKE ST NW'] }})
        ...
        db.Report.makeNeighbours('405 FAKE ST NW', 'asc', '371 FAKE ST NW', (err, msg) => {
          if (err) {
            return done.fail(err);
          }
          expect(msg).toEqual(`${results[0]['Location Address']} is up from ${results[1]['Location Address']}`);
          db.Report.find({'Location Address': {$in: ['405 FAKE ST NW', '371 FAKE ST NW']}})
                   .sort('Location Address').then(results => {
            expect(results[0]['Location Address']).toEqual('405 FAKE ST NW');
    ...
    it('sets a property\'s descending neighbour', done => {
      db.Report.find({ 'Location Address': { $in: ['405 FAKE ST NW', '371 FAKE ST NW'] }})
        ...
        db.Report.makeNeighbours('405 FAKE ST NW', 'desc', '371 FAKE ST NW', (err, msg) => {
          if (err) {
            return done.fail(err);
          }
          expect(msg).toEqual(`${results[0]['Location Address']} is down from ${results[1]['Location Address']}`);
          db.Report.find({'Location Address': {$in: ['405 FAKE ST NW', '371 FAKE ST NW']}})
                   .sort('Location Address').then(results => {
            expect(results[0]['Location Address']).toEqual('405 FAKE ST NW');
            expect(results[0]['Ascending Neighbour']).toEqual(results[1]._id);
